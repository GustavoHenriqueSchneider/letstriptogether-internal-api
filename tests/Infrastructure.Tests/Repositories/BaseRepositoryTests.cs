...
public class BaseRepositoryTests : TestBase
{
    ...
    [Test]
    public async Task Update_WithDetachedEntityNotInContext_ShouldAttachAndUpdate()
    {
        // Arrange
    ...
    [Test]
    public async Task Update_WithDetachedEntityInLocalCache_ShouldUpdateFromLocal()
    {
        // Arrange
        ...
        typeof(Role).GetProperty("Name")!.SetValue(newRole, newName);
    ...
    [Test]
    public async Task Update_ShouldCallSetUpdateAt()
    {
        // Arrange
        var role = new Role { Name = $"updAt{Guid.NewGuid().ToString()[..8]}" };
        await _repository.AddAsync(role, CancellationToken.None);
        await DbContext.SaveChangesAsync();
        ...
        // Act
        typeof(Role).GetProperty("Name")!.SetValue(role, $"newName{Guid.NewGuid().ToString()[..8]}");
        _repository.Update(role);
        await DbContext.SaveChangesAsync();
        ...
        {
            updated.UpdatedAt.Should().BeAfter(initialUpdatedAt.Value);
        }
    }
}
